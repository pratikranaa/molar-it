...
const extraStyles=['illustrations.css','application-scene.css','service-lab.css','identity.css','entry-experience.css','company.css','scenario-detail.css','analytics.css'].map(file=>`<link rel="stylesheet" href="/marketing/${file}?v=${revision('./'+file)}">`).join('');
const sceneVersion=revision('./application-scene.js');
export function document({title,description,path='/',body,faqs=[],type='WebPage',noindex=false,image,date,datePublished,dateModified,author,breadcrumbs,section,keywords}={}){
