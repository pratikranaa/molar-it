...
function cartographer(index) {
  return figure(index ? 'Verified flow evidence' : 'Browser journey evidence', `<div class="sd-flow-head"><span class="sd-kicker">Example path</span><code>signup / verify / workspace</code><span class="sd-status">${tick} observed</span></div><div class="sd-flow-steps">
...
function workflow() {
  return figure('Workflow document output', `<div class="sd-workflow-grid"><section class="sd-card"><span class="sd-kicker">Approved task</span><strong>Download latest invoice</strong><div class="sd-task-line">${tick}<span>Open billing portal · done</span></div><div class="sd-task-line">${tick}<span>Find invoice row · done</span></div><div class="sd-task-line"><span class="sd-dot"></span><span>Save PDF · next</span></div></section><div class="sd-map-arrow">${arrow}</div><section class="sd-card sd-file-card"><span class="sd-file-mark">PDF</span><strong>invoice-download</strong><span>Expected artifact</span><code>status · pending</code></section></div>`, 'sd-workflow');
}
...
function developer() {
  return figure('Developer verification loop', `<div class="sd-code-flow"><section class="sd-card"><span class="sd-kicker">Terminal</span><code>$ molar verify "login works"</code><code>  --url http://localhost:3000</code><code>  --json</code></section><div class="sd-map-arrow">${arrow}</div><section class="sd-card"><span class="sd-kicker">Run output</span><span class="sd-check-reason">Pass or failure plus available evidence</span><code>task status · trace reference</code><code>screenshots · captured evidence</code><span class="sd-status">${tick} inspect and rerun</span></section></div>`, 'sd-developer');
}
...
function outcomes() {
  return figure('Know what the result means', `<dl class="sd-outcomes"><div><dt>${icon('check')} Completed</dt><dd>The requested result was observed.</dd></div><div><dt>${icon('x')} Failed</dt><dd>The check did not reach its expected outcome.</dd></div><div><dt>${icon('lock')} Blocked</dt><dd>Access, a required input, or a workflow limit prevented progress.</dd></div></dl><p class="sd-controlled-note">Inspect the available steps and evidence before deciding what to do next.</p>`, 'sd-outcome-scene', 'Run outcomes');
}
function repairFlow() {
  return figure('Review the repair before applying it', `<ol class="sd-review-path">${[['trace','Start with the failure','Keep the failing assertion and its evidence.'],['code','Propose an application change','Attach the candidate patch for review.'],['users','Review the proposal','Confirm the change and its scope.'],['replay','Check the repair','Rerun the original check; keep the result.']].map(([i,n,d])=>`<li>${icon(i)}<div><strong>${n}</strong><p>${d}</p></div></li>`).join('')}</ol>`, 'sd-repair-path', 'Mender preview workflow');
}
