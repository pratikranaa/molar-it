...
The Clones catalog is an open two-column searchable list of 28 entries: 5 stateful service clones and 23 API service clones. Each row exposes the provider name, coverage tier, operation summary, and a link to its existing documentation. Provider marks come from the local brand source manifest and attribution files; Auth uses the generic lock treatment and niche providers use their official site favicons. The catalog filter has All services (28), Stateful (5), and API clones (23), plus an operation-aware search and an explicit empty state.

The startup strip says “Supported by startup programs” and shows Google for Startups Cloud Program, Microsoft for Startups, AWS Activate, and Razorpay Rize. These marks and links are part of the relationship copy; they do not change the established paper, sage, coral, and green-black identity.
...
- **Do** keep the full 28-entry catalog searchable by provider and operation, with the operation coverage available when customers inspect each clone.
- **Do** use the actual local provider marks and describe startup-program membership without emphasizing credits or implying investment.
...
### Signup and Clones entry copy — September 7, 2026

The desktop/mobile header and homepage/platform primary actions say “Get started” and link to dashboard signup. The hero keeps its interactive example as the second action and an explicit no-account public-page check beneath it. The project command remains in the action column. Generic closing sections lead to signup; sections specifically offering a public check retain their matching destination.
